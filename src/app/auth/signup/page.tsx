...
function SignupContent() {
  ...
  const handleQuickDemo = (demoRole: "attendee" | "admin") => {
    if (demoRole === "admin") {
      signup("admin@vibepass.com", "Admin Joey", "admin");
      router.push("/admin");
    } else {
      signup("alex@vibepass.com", "Alex Mercer", "attendee");
    ...
    <div className="bg-background text-on-background min-h-screen flex flex-col pt-16 pb-12 selection:bg-primary-container selection:text-on-primary-container">
      ...
      <main className="flex-1 flex flex-col justify-center px-4 max-w-[440px] mx-auto w-full py-4">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-2.5">
          ...
          {/* 2FA Protection */}
          <div className="bg-surface-container-low rounded-xl p-3 border border-outline-variant/40 mt-0.5">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-xs font-semibold text-on-surface flex items-center gap-1">
                ...
                <span>Protect account with SMS Two-Factor Auth</span>
              </span>
            </label>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="mt-2 w-full bg-primary text-on-primary font-[family-name:var(--font-montserrat)] text-sm font-bold py-3.5 rounded-xl shadow-md hover:bg-on-primary-fixed-variant active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
            {isLoading ? (
              <span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
            ) : (
              <span>Create {role === "admin" ? "Organizer Portal" : "Account"}</span>
            )}
          </button>
        </form>
